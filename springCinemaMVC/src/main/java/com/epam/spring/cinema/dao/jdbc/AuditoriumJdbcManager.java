...
public class AuditoriumJdbcManager implements AuditoriumManager {
    ...
    private final String GET_AUDITORIUM_BY_EVENT_ID = "SELECT AUD.ID AS ID, AUD.NAME AS NAME, AUD.NUMBER_OF_SEATS AS NUMBER_OF_SEATS, AUD.VIP_SEATS AS VIP_SEATS FROM CINEMA_EVENT_AUDITORIUM_LINK LN INNER JOIN CINEMA_AUDITORIUM AUD ON AUD.ID = LN.AUDITORIUM_ID WHERE LN.EVENT_ID = ?";
    ...
    public Auditorium getAuditoriumByEventId(Long eventId) {
        return jdbcTemplate.queryForObject(GET_AUDITORIUM_BY_EVENT_ID, new Object[]{eventId}, new AuditoriumRowMapper());
    }
}
