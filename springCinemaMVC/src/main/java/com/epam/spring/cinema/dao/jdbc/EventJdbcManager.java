...
public class EventJdbcManager implements EventManager {
    ...
    private final String INSERT_QUERY = "INSERT INTO CINEMA_EVENT (ID, NAME, BASE_PRICE, RATIN_SYS_NAME, EVENT_DATE, DESCRIPTION) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    private final String GET_BY_ID_QUERY = "SELECT * FROM CINEMA_EVENT WHERE ID = ?";
    ...
    public void save(Event event) {
        ...
        jdbcTemplate.update(INSERT_QUERY, id, event.getName(), event.getBasePrice(), event.getRating().getSysName(), date, event.getDescription());

        jdbcTemplate.update(INSERT_AUDITORIUM_LINK_QUERY, id, new java.sql.Date(date.getTime()), event.getAuditorium().getId());
